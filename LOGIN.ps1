    ...
    if(![System.Environment]::GetEnvironmentVariable('TWBINROOT','MACHINE')){
        ...
        write-host "`t%TWBINROOT%" -ForegroundColor blue
    }
    if(![System.Environment]::GetEnvironmentVariable('TWDOWNLOAD','MACHINE')){
        write-host "`nand`n" -foregroundcolor yellow
        write-host "`t%TWDOWNLOAD%`n" -foregroundcolor blue
    }
...
"@ -ForegroundColor Yellow
    ...
    write-host " -Params " -ForegroundColor Blue -NoNewLine
    write-host "`$GLOBAL:TWPARAMS " -foreGroundColor Green -NoNewLine
