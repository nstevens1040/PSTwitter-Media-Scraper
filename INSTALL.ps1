...
if(!$PWD.Path.Contains("PSTwitter-Media-Scraper")){
    ...
    while(Compare-Object $ReposFiles $LocalFiles){
        ...
        Write-Host "Rename " -ForegroundColor Yellow -NoNewline
        Write-Host "$($CDIR)" -ForegroundColor Green -NoNewline
        write-host " to " -ForegroundColor Yellow -NoNewline
        Write-Host "$($DEST)" -ForegroundColor Green -NoNewline
        write-host " and re-clone " -ForeGroundColor Yellow -NoNewLine
