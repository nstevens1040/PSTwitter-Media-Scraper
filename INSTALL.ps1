    ...
    if(!$PWD.Path.Contains("PSTwitter-Media-Scraper")){
        ...
        while(Compare-Object $ReposFiles $LocalFiles){
            ...
            Write-Host "Delete the contents of:" -ForegroundColor Yellow
            Write-Host "`t$($CDIR)" -ForegroundColor Green
            write-host "and reset the local repo for:" -ForeGroundColor Yellow
