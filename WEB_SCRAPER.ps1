        ...
        if($ans -eq 'y'){
            $null = ([System.Diagnostics.Process]@{
                StartInfo = [System.Diagnostics.ProcessStartInfo]@{
                    ...
                    Arguments = " -ep RemoteSigned -noprofile -nologo -c cd '$($PWD.Path)'; iex (irm 'https://raw.githubusercontent.com/nstevens1040/PSTwitter-Media-Scraper/master/INSTALL.ps1'); Install-PSTwitterMediaScraper -TARGET_URI '$($TARGET_URI)"
                };
            }).Start()
            ...
            if($MyInvocation.MyCommand.Path){
                ...
                if($ans -eq 'y'){
                    $null = ([System.Diagnostics.Process]@{
                        StartInfo = [System.Diagnostics.ProcessStartInfo]@{
                            FileName = "$($PSHOME)\PowerShell.exe";
                            Arguments = " -ep RemoteSigned -noprofile -nologo -c cd '$($GLOBAL:CDIR)'; iex (irm 'https://raw.githubusercontent.com/nstevens1040/PSTwitter-Media-Scraper/master/INSTALL.ps1'); Install-PSTwitterMediaScraper -TARGET_URI '$($TARGET_URI)'"
                        };
                    }).Start()
                    (Get-Process -Id $PID).kill()
                }
...
{
    Param(
        [ValidateSet('GET','POST','HEAD','OPTIONS','PUT')]
        [String]$METHOD,
        [String]$BODY,
    ...
    switch($METHOD){
        ...
        "PUT" {
            if($CONTENT_TYPE){
                if(!$BODY){
                    $RM = [System.Net.Http.HttpRequestMessage]::new([System.Net.Http.HttpMethod]::Put,"$($URI)")
                    if($FILE){
        ...
        "OPTIONS" {
            $RM = [System.Net.Http.HttpRequestMessage]::new([System.Net.Http.HttpMethod]::Options,"$($URI)")
            $RES = $CLIENT.SendAsync($RM)
            $RESHEAD = $RES.Result.Headers
        }
    }
