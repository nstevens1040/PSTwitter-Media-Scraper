function Scrape-TWPage
{
    ...
    Param(
        [string]$TARGET_URI,
        [bool]$Skip_RedirectionChecks,
        [switch]$ReEncode_Videos
    )
    ...
    function Get-TwMediaUris
    {
        ...
        if($MLINKS){
            if($MLINKS.Count -eq 1){
                $CURRENT = $MLINKS[0]
                if(!$Skip_RedirectionChecks){
                    $SINGLE = @($MLINKS[0].Split("`n")).Where({$_.Contains("http")}) | Detect-Redirect
                } else {
                    $SINGLE = @($MLINKS[0].Split("`n")).Where({$_.Contains("http")})
                }
                if(
                    !$SINGLE -and `
                    $LINKS.IndexOf($SINGLE) -ne -1
                ){ } else {
                    $LINKS += $SINGLE
                }
            } else {
                $MLINKS.forEach({
                    ...
                    $current = $_
                    if($current -match [char]32 -or $current -match [char]10){
                        $u = @()
                        @($current.split([char]32)).ForEach({
                            @($_.split([char]10)).forEach({
                                $u += $_
                            })
                        })
                    }
            ...
            if($LINKS.GetType() -eq [string]){
                $r = $true
                return $LINKS
            }
            if($LINKS.GetType() -eq [object[]]){
                $r = $true
                return (, $LINKS)
            }
            if(!$r){
    ...
    function MakeRequestBody
    {
        ...
        $JSON = '{ "redirect_after_login": "/", "remember_me": "1", "authenticity_token": "--MYAUTHTOKEN--", "wfa": "1", "ui_metrics": "{\"rf\":{\"af9cb77540720722484f6d166893a8b6f053ec70b3540ce5d69c18343f5aa5dc\":-8,\"a84b7fcba8eac67b3458cf40fd7aad5d923ae1a34f71e2620f0523cb91b3ef18\":0,\"acf3d178b008a3ec2fc6acd748a8c9dd97ee6e35e1ad60b75f7e030ed56ddc43\":155,\"cc4e1fbb01997b73f291bc4b9df4eb22605776b33ee9af509195308d95b45259\":-1},\"s\":\"maF93FMzZK26SZuyezHO2KFsplPJfBMAiqRSLlUdIOHQqfpkNbzBK-e9S_ZyveyGltMputA3N6-OGQl1ubkVqEz95ajMW02mGd7tZ0QISL7N1kUJ5uKwdwSrRpMC2mLtvwp0v66WmdZ20P0Foe2yST7JMb4zQ8EH46NjarHtyPvERSL7960N93B16zWH_LcPXQcx06-kwoI-HDhz1WvfY9X9jCjp7iwDw-yYY5yxr5KQodTB9nArDAQen-tMoZFLRxenlO4H5YpbHK_DOsoMmZDDlXPFGx4FBVNn-KTBqjkxVuVHV_vd8CVaJJNM_wPFSu-ajZYcvLMBaekjJMTZpwAAAXN6CcXQ\"}", "session%5Busername_or_email%5D": "' + "$($CREDRA[0])" + '", "session%5Bpassword%5D": "' + "$($CREDRA[1])" + '"}'
    ...
    {    
        ...
        While(!$USERNAME){
            ...
            $INIT = Execute-WebRequest -METHOD GET `
            -URI "https://twitter.com/login" `
            -GET_REDIRECT_URI

            $NOJSURI = @(@($INIT.ResponseText.split("`n")).Where({$_ -match 'form'})[0].split('"')).Where({$_ -match 'https'})

            $NOJS = Execute-WebRequest -METHOD POST `
            -URI $NOJSURI `
            -CONTENT_TYPE "application/x-www-form-urlencoded" `
            -HEADERS $INIT.HttpResponseHeaders `
            -DEFAULTCOOKIES $INIT.CookieCollection `
            -GET_REDIRECT_URI `
            -REFERER "https://twitter.com/login"

            $MOBILE = Execute-WebRequest -METHOD GET `
            -HEADERS $NOJS.HttpResponseHeaders `
            -DEFAULTCOOKIES $NOJS.CookieCollection `
            -GET_REDIRECT_URI `
            -URI $NOJS.RedirectUri

            $FORMS = @(); @($MOBILE.HtmlDocument.body.getElementsByTagName("FORM")).ForEach({ $FORMS += $_ })

            $UIMET = Execute-WebRequest -Method GET -Uri "https://mobile.twitter.com/i/js_inst?c_name=ui_metrics"
            ...
            $COLLECT.Add($COOKIE1)
            $COLLECT.Add($COOKIE2)
            $MOBILE.CookieCollection.ForEach({ $COLLECT.Add($_) })
        ...
        $CSRF = [System.Convert]::ToBase64String(
            [System.Security.Cryptography.ProtectedData]::Protect(
                [System.Text.Encoding]::Unicode.GetBytes(
                    @($REDIRECT.CookieCollection).Where({ $_.Name.Equals('u') }).Value
                ),
                $null,
                [System.Security.Cryptography.DataProtectionScope]::LocalMachine
            )
        )
        $REDIRECT.CookieCollection.Add(
            [System.Net.Cookie]@{
                HttpOnly = $false;
                Discard = $false;
                Domain = ".twitter.com";
                Expired = $false
                Expires = $REDIRECT.CookieCollection.Where({$_.Name.Equals('u')})[0].Expires;
                Name = 'ct0';
                Path = "/";
                Secure = $true;
                Value = $REDIRECT.CookieCollection.Where({$_.Name.Equals('u')})[0].Value;
            }
        )
        $GLOBAL:TWPARAMS = @{
            BEARER=$BEARER_TOKEN;
    ...
    $webheadercollection += [ordered]@{
        ...
        "sec-fetch-mode"="cors";
        "sec-fetch-dest"="empty";
        "referer"="https://mobile.twitter.com/home";
        "accept-encoding"="gzip, deflate";
        "accept-language"="en-US,en;q=0.9";
    }
    if(!$GLOBAL:TWPARAMS){
    ...
    $USER = Execute-WebRequest -Method GET `
         -NO_COOKIE `
         -BEARER $BEARER_TOKEN `
         -Uri "https://api.twitter.com/1.1/users/lookup.json?screen_name=$($HANDLE)"
    ...
    Remove-Variable -Name LINKS,done -ea 0
    $LINKS = @()
    $GLOBAL:FINDMEDIA = @()
    $GLOBAL:TWTS = $TWEETS.ResponseText | ConvertFrom-Json | % globalObjects | % tweets
    $TWEETIDS = @()
    $TWEETIDS += @($TWTS | gm -MemberType NoteProperty | % Name)
    $bwstart = [Console]::BufferWidth
    [console]::bufferWidth = [Console]::WindowWidth
    $done = @()
    $start = [datetime]::now
    for($i = 0; $i -lt $TWEETIDS.Count; $i++){
        if(!$Skip_RedirectionChecks){
            $job = Start-Job ${function:Get-TwMediaUris} -Name "$($TWEETIDS[$i])" -ArgumentList @(($GLOBAL:TWTS | % "$($TWEETIDS[$i])"),$false) 
        } else {
            $job = Start-Job ${function:Get-TwMediaUris} -Name "$($TWEETIDS[$i])" -ArgumentList @(($GLOBAL:TWTS | % "$($TWEETIDS[$i])"),$true) 
        }
        $completed = @((Get-Job).Where({$_.State -eq 'Completed'}))
        $running = @((Get-Job).Where({$_.State -ne 'Completed'})).Count
        while(@((Get-Job).Where({$_.State -ne 'Completed'})).Count -ge 60){ sleep -m 500 }
        $completed.Where({$_.Name -notin $done}).ForEach({
            $done += "$($_ | % Name)"
            $out = ($_ | % childjobs | % output)
            if(!$out){
                $GLOBAL:FINDMEDIA += ($GLOBAL:TWTS | % "$($_ | % Name)")
            } else {
                if($out.GetType() -eq [string]){
                    $linkstring = $out
                    while($linkstring -match "([^\n])https(.*)"){
                        $linkstring = "$($linkstring -replace "([^\n])https(.*)","`$1`nhttps`$2")"
                    }
                    if($LINKS){
                        @($linkstring.split("`n")).where({$LINKS.IndexOf($_) -eq -1}).forEach({$LINKS += $_})
                    }
                    if(!$LINKS){
                        @($linkstring.split("`n")).ForEach({$LINKS += $_ })
                    }
                }
                if($out.GetType() -eq [System.Collections.ArrayList]){
                    $out.forEach({
                        $linkstring = $_
                        while($linkstring -match "([^\n])https(.*)"){
                            $linkstring = "$($linkstring -replace "([^\n])https(.*)","`$1`nhttps`$2")"
        ...
        if($i -ge 1){
            $n = [datetime]::Now
            $e = ($n - $start).totalSeconds
            $r = ($e*($TWEETIDS.Count/$i)) - $e
            ($n.AddSeconds($r) - $n) | select Days,hours,Minutes,Seconds,Milliseconds | % { $ts = "$($_ | % days) days :: $($_ | % hours) hours :: $($_ | % minutes) minutes :: $($_ | % seconds) seconds ::$($_ | % milliseconds)ms" }
            Write-Progress -PercentComplete (($i+1)/$TWEETIDS.Count*100) -Status "$($ts) :: $([math]::Round(($completed.Count/$TWEETIDS.Count*100),2))%" -Activity "$($LINKS.Count) links parsed :: $((Get-Job).Count) total jobs :: $($completed.Count) jobs completed :: $($running) jobs running and $($i) of $($TWEETIDS.Count) tweets checked"
        }
    }
    write-progress -Completed
    Remove-Variable done -ea 0
    Get-Job | Remove-Job
    $START = Get-Date
    $COUNT = 0
    $ALL = $LINKS.Count
        ...
        try {
            switch ($pre){
                ...
                "https://video.twimg.com" { 
                    if($ReEncode_Videos){
                        Download-Video -VIDEOURL $uri -CSRF $CSRF -TWROOT $TWROOT -BEARER_TOKEN $BEARER_TOKEN -REENCODE_VIDEOS
                    } else {
                        Download-Video -VIDEOURL $uri -CSRF $CSRF -TWROOT $TWROOT -BEARER_TOKEN $BEARER_TOKEN
                    }
                    $vidCount++ 
                }
