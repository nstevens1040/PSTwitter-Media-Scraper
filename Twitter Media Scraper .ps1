...
"@ 
        ...
        if($SECONDARY){
            $BEARER = [Dialog.Prompt]::ShowDialog("Please paste your secondary Bearer token and click 'Ok'.","Twitter Scraping Machine")
        } else {
            $BEARER = [Dialog.Prompt]::ShowDialog("Please paste your Bearer token and click 'Ok'.","Twitter Scraping Machine")
        }
        return $BEARER
    }
...
"@
    ...
    AddAllAssemblies
    if($BEARER1){ $BEARER_TOKEN = $BEARER1 }
    if(!$BEARER_TOKEN){
        $BEARER_TOKEN = Get-TWBearerToken
    }
    if($BEARER2){ $SECONDARY_BEARER = $BEARER2 }
    if(!$SECONDARY_BEARER){
        if(
    ...
    $CSRF =  @($REDIRECT.CookieCollection).Where({$_.Name.Contains("ct0")}).Value
    $WebHeaderCollection += [ordered]@{
        "authorization"="Bearer $($SECONDARY_BEARER)"; 
        "x-csrf-token"= "$($CSRF)";
        ...
        if(!$VIDEOURL){
            ...
            if($MEDIA){
                ...
                if(@($MEDIA.split(' ')).Length -gt 1){
                    ...
                    @($MEDIA.split(' ')).ForEach({
                        $ImgCount++
                        Download-Image -MEDIAURL $_ -TWROOT $TWROOT -LINK "$($LINK)_$($c)" -BEARER_TOKEN $BEARER_TOKEN
                        $C++
                    })
                } else {
                    ...
                    $ImgCount++
                    Download-Image -MEDIAURL $MEDIA -TWROOT $TWROOT -LINK $LINK -BEARER_TOKEN $BEARER_TOKEN
                }
            }
            if(!$MEDIA){
