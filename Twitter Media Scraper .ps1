...
"@
    ...
    {
        Param()
        $cfi = [Window.Native.Kernel32]::GetCurrentConsoleFontEx()
        $cfi.FontHeight = 14
        $cfi.FontWidth = 8
        [Window.Native.Kernel32]::SetCurrentConsoleFontEx($cfi)
...
"@
    ...
    function Download-Image
    {
        ...
        if($MEDIAURL.Contains("twimg")){
            ...
            try {
                ...
                $e = $
                $MEDIAURL | Out-File C:\TEMP\SOCIAL\TWITTER\TW_BUCKET\Exceptions.txt -Encoding ascii -Append 
                $e.Exception | % { $_ | out-file C:\TEMP\SOCIAL\TWITTER\TW_BUCKET\Exceptions.txt -Encoding ascii -Append }
            }
        }
    ...
    function Download-Video
    {
        ...
        if($VIDEOURL.Contains("mp4")){
            ...
            try {
                ...
                $e = $_
                $VIDEOURL | out-file C:\TEMP\SOCIAL\TWITTER\TW_BUCKET\Exceptions.txt -Encoding ascii -Append
                $e.Exception | % { $_ | out-file C:\TEMP\SOCIAL\TWITTER\TW_BUCKET\Exceptions.txt -Encoding ascii -Append }
            }
    ...
    function Get-VideoUrl
    {
        ...
        if($URL){
            if($MORE.GetType() -eq [System.Object[]]){
                "##### CHOSE $($URL) #####" | Out-File C:\TEMP\SOCIAL\TWITTER\VIDEO_DOWNLOADS.txt -Encoding ascii -Append
                $MORE.forEach({
                    "$($_)" | Out-File C:\TEMP\SOCIAL\TWITTER\VIDEO_DOWNLOADS.txt -Encoding ascii -Append
                })
                "##########`n" | Out-File C:\TEMP\SOCIAL\TWITTER\VIDEO_DOWNLOADS.txt -Encoding ascii -Append
            } else {
                "##### ONE CHOICE #####" | Out-File C:\TEMP\SOCIAL\TWITTER\VIDEO_DOWNLOADS.txt -Encoding ascii -Append
                "$($URL)"  | Out-File C:\TEMP\SOCIAL\TWITTER\VIDEO_DOWNLOADS.txt -Encoding ascii -Append
                "##########" | Out-File C:\TEMP\SOCIAL\TWITTER\VIDEO_DOWNLOADS.txt -Encoding ascii -Append
            }
            return $URL
        }
    }
    ...
    if(
        $PAGE2SCRAPE.split('/')[0] -ne 'https:' -or `
        $PAGE2SCRAPE.split('/')[2] -ne 'twitter.com' -or `
        !$PAGE2SCRAPE.split('/')[3]
    ){
        $MALFORMED = $true
    }
    while($MALFORMED){
        ...
        $PAGE2SCRAPE = TW-TargetPage
        if(
            $PAGE2SCRAPE.split('/')[0] -eq 'https:' -and `
            !$PAGE2SCRAPE.split('/')[1] -and `
            $PAGE2SCRAPE.split('/')[2] -eq 'twitter.com' -and `
            $PAGE2SCRAPE.split('/')[3] -ne $null
        ){
            $MALFORMED = $false
        }
    }
    
    $HANDLE = $PAGE2SCRAPE.Split('/')[[array]::IndexOf(@($PAGE2SCRAPE.Split('/')),"twitter.com") + 1]
    ...
    while (!$USERNAME) {
        ...
        $mediaLink = "https://twitter.com/$(($PAGE2SCRAPE).Split('/')[3])/media"
        $WebHeaderCollection = @()
    ...
    $LINKS.ForEach({
        ...
        if($rd){
            try {
                ...
                $e = $_
                "HTTP HEAD request failed for $($n)" | out-File C:\TEMP\SOCIAL\TWITTER\TW_BUCKET\Exceptions.txt -Encoding ascii -Append 
                $e.Exception | % { $_ | out-File C:\TEMP\SOCIAL\TWITTER\TW_BUCKET\Exceptions.txt -Encoding ascii -Append }
            }
            if($r){ 
